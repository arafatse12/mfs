...
class Footer_submenu extends CI_Controller
{
        ...
        if ($this->input->post('form_submit')) {
            if ($this->data['admin_id'] > 1) {
             ...
             $data['footer_menu']    = $this->input->post('main_menu');
                ...
                $this->session->set_flashdata('success_message', 'Footer Settings updated successfully');
    ...
    public function contact_widget() {
     ...
     public function copyright_widget() {
            ...
            // $languages = $query->result();
            // foreach ($languages as $language) {
            //     $labelss = $this->input->post('label_' . $language->id);
            //     foreach ($labelss as $lab) {                
            //     $copy_wid_val = array(
            //         'modules' => 'copyright-widget',
            //         'label' => $lab,
            //         'lang_type' => $language->language_value
            //     );
                
            //     $this->db->where('modules', 'copyright-widget');
            //     $this->db->where('lang_type', $language->language_value);
            //     $row = $this->db->get('footer_submenu_lang')->row_array();
            //     if (empty($row)) {
            //         $this->db->insert('footer_submenu_lang', $copy_wid_val);
            //     } else {
            //         $this->db->where('modules', 'copyright-widget');
            //         $this->db->where('lang_type', $language->language_value);
            //         $this->db->update('footer_submenu_lang', $copy_wid_val);
            //     }
