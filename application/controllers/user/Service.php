...
class Service extends CI_Controller {
    ...
    public function __construct() {
        ...
        // Array with the settings for this instance of CKEditor (you can have more than one)
        $this->data['ckeditor_editor5'] = array(
            //id of the textarea being replaced by CKEditor
            'id' => 'ck_editor_textarea_id5',
            // CKEditor path from the folder on the root folder of CodeIgniter
            'path' => 'assets/js/ckeditor',
            // optional settings
            'config' => array(
    ...
    public function add_service() {  
        if($this->session->userdata('usertype') == 'provider') {
            ...
            $this->session->set_flashdata('error_message', 'You are not allowed this page');
            redirect(base_url());
        }
    }
         ...
         $result=$this->service->update_bookingstatus($book_details,$WHERE);
		  ...
		  if($result){
           ...
           if($book_details['status'] ==6){
             ...
             $token=$this->session->userdata('chat_token');
             $complete_accept_text = (!empty($this->user_language[$this->user_selected]['lg_have_completed_the_service'])) ? $this->user_language[$this->user_selected]['lg_have_completed_the_service'] : $this->default_language['en']['lg_have_completed_the_service'];
...
//echo "<pre>";print_r($availability_details);exit;
        $alldays = false;
        ...
        if(date('G:i:s', ($timestamp_end))=='23:30:00'){
            $to_time_railwayhrs = date('G:i:s', ($timestamp_end)- 30 * 60 * 1);
        }else{
      ...
      //  echo "<pre>";print_r($timing_array);exit;
     ...
     /* fix for availability 30 min extra problem in to time */
     $contain_st= $this->string_contain($temp_start_time, "30") ? true : false;
     $contain_end= $this->string_contain($temp_end_time, "30") ? true : false;
     if(date('G:i:s', ($timestamp_end))=='23:30:00' && $contain_st==false){
        array_push($new_timingarray,['start_time'=>'23:00:00','end_time'=>'23:30:00']);
      }else{
        if($contain_st == false && $contain_end==true){
            $end_time_arr=end($new_timingarray);
            $end_time_change=$end_time_arr['end_time'];
            $end_timestamp_railwayhrs=strtotime($end_time_change);
            $key_value=key($new_timingarray);
            $changed_end_time=date('G:i:s', (($end_timestamp_railwayhrs) - 60 * 30 * 1));
            $new_timingarray[$key_value]=['start_time'=>$end_time_arr['start_time'],'end_time'=>$changed_end_time];
        }
        if($contain_st == true && $contain_end==false){
            $end_time_arr=end($new_timingarray);
            $end_time_change=$end_time_arr['end_time'];
            $end_timestamp_railwayhrs=strtotime($end_time_change);
            $key_value=key($new_timingarray);
    ...
    public function string_contain($str,$contain){
