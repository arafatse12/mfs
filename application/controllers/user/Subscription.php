...
class Subscription extends CI_Controller {
   ...
   public function __construct() {
        ...
        $subscriped_text = (!empty($this->user_language[$this->user_selected]['lg_have_been_subscribed'])) ? $this->user_language[$this->user_selected]['lg_have_been_subscribed'] : $this->default_language['en']['lg_have_been_subscribed'];
    ...
    if($result){
       ...
       $this->send_push_notification($token,$this->session->userdata('id'),1,' '.$subscriped_text); 
          }else{
  ...
  //Paystack payment 
    public function paystack_payment_success() { 
        ...
        $inputs['user_id'] = $this->session->userdata('id');
        $inputs['token']=$this->session->userdata('chat_token');
        $inputs['payment_details']='Paystack';
