...
else
{
    ...
    public function user_wallet()
    {
        ...
        if($flutter_option == 1){     
            $this->data['flutter_apikey']=settingValue('flutter_apikey');
        }else if($flutter_option == 2){
            $this->data['flutter_apikey']=settingValue('live_flutter_apikey');
        }
...
echo json_encode($result);
    ...
    public function update_payout_settings()
    {
        $id=$this->session->userdata('id');
        removeTag($this->input->post());
            ...
            $data=array('tab_ctrl'=>1,'success_message'=>'Payout updated successfully');
            $this->session->set_flashdata($data);
            redirect(base_url()."payout-settings");   
    }

public function user_reviews()
{
 $this->data['page'] = 'user_reviews';
$this->db->select("r.*,u.profile_img,u.name,s.service_image,s.service_title,s.url");
...
{
 $this->data['page'] = 'payout_settings';
 $this->load->vars($this->data);
 $this->load->view($this->data['theme'].'/template');
}
...
public function provider_subscription()
{
   ...
   $this->data['midtrans_option_status']=settingValue('midtrans_show'); 
...
public function iyzico_payment(){
	...
	$this->data['user']     = $this->db->where('id', $user_id)->get('users')->row_array();
   
    $this->data['amount'] = $amount;
    $this->data['iyzico_apikey'] = settingValue('iyzico_apikey');
    $this->data['iyzico_secret_apikey'] = settingValue('iyzico_secret_apikey');



    $this->load->vars($this->data);
    $this->load->view('user/home/iyzico');
   
}

public function iyzico_withdraw(){
  
    $amount = $this->uri->segment(4);
    $user_id = $this->session->userdata('id');
   

	$this->data['user']     = $this->db->where('id', $user_id)->get('providers')->row_array();
    ...
    $this->load->vars($this->data);
    $this->load->view('user/home/iyzico_payout');
   
}



public function iyzico_provider_subscription(){
...
public function iyzico_payment_post(){
    ...
    $options->setApiKey('sandbox-CodtRKflRLBhqRYJlhr6gEAAigPSs8aW');
    $options->setSecretKey('sandbox-i5k09CjwjzEEQ1BfHCmmPdUPENQTCIPW');
    $options->setBaseUrl("https://sandbox-api.iyzipay.com");
        ...
        if($checkoutForm->getPaymentStatus() == "SUCCESS"){
            ...
            if($usertType == '2') {
                ...
                $history_pay['payment_type']='iyzico';
...
//paramesh
        ...
        if (!empty($params) && ($user_id == $userId)) {
            ...
            if ($check_bank > 0) {            
                ...
                $contact_data = array(
                    ...
                    'accountnumber' => $xrazor_acc_number,
                    'mode' => $user_acc_det['mode'],
                    'purpose' => $user_acc_det['purpose'],
                ...
                if(!empty($createcontact)){
                    ...
                    if($facreatecontact){
                        ...
                        if(!empty($presults->error->description)){
                           echo json_encode(array(
                               'status' => false,
    ...
    //updated on 24/07/2021
    public function razorpay_details1()
    {
        removeTag($this->input->post());
        $params        = $this->input->post();
        $user_id       = $this->session->userdata('id');
...
public function service_map_list(){
  ...
  $this->db->where('subscriber_id', $pro_id);
    ...
    $history_pay['sub_id'] = $this->db->insert_id();
        ...
        $this->db->where('subscriber_id', $pro_id);
        $this->db->update('subscription_details', $new_details);
        $this->db->update('subscription_details_history', $new_details);
    $this->db->where('subscriber_id', $pro_id);
        $details_sub = $this->db->get('subscription_details')->row_array();
...
//flutter
    ...
    public function midtrans_withdraw() {

        $user_data = $this->input->post();
        $user_id       = $this->session->userdata('id');
        $userId = $user_data['user_id'];
        $apikey = "IRIS-bd78ab99-dcc3-4cff-aa88-dd68fff0cdd7";
       ...
       //echo "<pre>"; print_r($payouts_data[0]->status);exit;
        $user_id = $this->session->userdata('id');
        ...
        if($createpayout > 0) {
            ...
            $history_pay['debit_wallet'] = $amount;
            $history_pay['type']='1';
            $history_pay['transaction_id']=$result;
            $history_pay['paid_status']='pending';
            $history_pay['cust_id']='self';
            $history_pay['card_id']='self';
            $history_pay['reference_key']=$refno;
            $history_pay['total_amt']=$withdraw_org_amt;
            ...
            $history_pay['created_at'] = date('Y-m-d H:i:s');
            $midhis =$this->db->insert('wallet_transaction_history',$history_pay);
    ...
    public function midtrans_payout_statuscheck() {
        $query = $this->db->query("select * from wallet_transaction_history WHERE payment_detail = 'midtrans' and paid_status='pending'");
        $result = $query->result_array();
        if(!empty($result)) { 
            foreach($result as $data1){
                if($data1['paid_status'] == 'pending'){
                 $refno = $data1['reference_key'];
               }
        ...
        if($result1->status=='completed'|| $result1->status=='approved'){
           $withdraw_org_amt = get_gigs_currency($result1->amount, $conversion_currency, $user_currency_code->currency_code);
            $withdraw_org_amount = round($withdraw_org_amt / 100);
            $amount=$result1->amount;
            $user_id       = $this->session->userdata('id');
            ...
            $withdrawaldata = array(
             ...
             //   echo $this->db->last_query();  exit;
                if($this->db->affected_rows() > 0){                               
                      ...
                      //  echo $this->db->last_query();  exit;
                         $this->db->where('token', $user_token)->where('reference_key', $refno)->update('wallet_transaction_history', array(
                            'paid_status' => '1',
                        ));
