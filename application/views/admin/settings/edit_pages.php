            ...
            <div class="col-xl-8 offset-xl-2">
                ...
                <div class="page-header">
                    <div class="row">
                        <div class="col">
                            <h3 class="page-title">
                                <?php echo(!empty($pages['lg_admin_edit_pages']))?($pages['lg_admin_edit_pages']) : 'Edit Pages';  ?>
                            </h3>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="<?php echo base_url().'admin/settings/edit_pages/' . $pages_val['id']; ?>"
                            id="add_pages" method="post" autocomplete="off" enctype="multipart/form-data">
                            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>"
                                value="<?php echo $this->security->get_csrf_hash(); ?>" />
                            <div class="form-group">
                                <label><?php echo(!empty($pages['lg_admin_title']))?($pages['lg_admin_title']) : 'Titles';  ?></label>
                                <input class="form-control" type="text" name="title" id="title"
                                    value="<?php echo $pages_val['title']; ?>">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="pages_desc" id="pages_desc"
                                    value="<?php echo $pages_val['description']; ?>">
                            </div>
                            <div class="form-group">
                                <label><?php echo(!empty($pages['lg_admin_keywords']))?($pages['lg_admin_keywords']) : 'Keywords';  ?>
                                    <small><?php echo(!empty($pages['lg_admin_meta_tag']))?($pages['lg_admin_meta_tag']) : '(Meta Tag)';  ?></small></label>
                                <input class="form-control" type="text" name="pages_key" id="pages_desc"
                                    value="<?php echo $pages_val['keywords']; ?>">
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select" name="pages_lang" id="pages_lang">
                                    ...
                                    <option value="<?php echo $rows['id'];?>"
                                        <?php if($rows['id']==$pages_val['lang_id']) echo 'selected';?>>
                                        <?php echo $rows['language'];?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label><?php echo(!empty($pages['lg_admin_location']))?($pages['lg_admin_location']) : 'Location';  ?></label><br>
                                <label><input type="radio" name="pages_loc" value="1"
                                        <?=(!empty($pages_val['location'])&&$pages_val['location']==1)?'checked':'';?>>
                                    <?php echo(!empty($pages['lg_admin_top_menu']))?($pages['lg_admin_top_menu']) : 'Top Menu';  ?>
                                </label>&nbsp
                                <label><input type="radio" name="pages_loc" value="2"
                                        <?=(!empty($pages_val['location'])&&$pages_val['location']==2)?'checked':'';?>>
                            ...
                            <div class="form-group">
                                ...
                                <label><input type="radio" name="pages_visibility" value="1"
                                        <?=(!empty($pages_val['visibility'])&&$pages_val['visibility']==1)?'checked':'';?>>
                                    <?php echo(!empty($pages['lg_admin_show']))?($pages['lg_admin_show']) : 'Show';  ?>
                                </label>&nbsp
                                <label><input type="radio" name="pages_visibility" value="2"
                                        <?=(!empty($pages_val['visibility'])&&$pages_val['visibility']==2)?'checked':'';?>>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class='form-control' id='ck_editor_textarea_id' rows='6'
                                    name="content"><?php echo $pages_val['page_content']; ?></textarea>
                                <?php echo display_ckeditor($ckeditor_editor1); ?>
                            </div>
                            <div class="mt-4">
                                <button class="btn btn-primary " name="form_submit" value="submit"
                                    type="submit"><?php echo(!empty($pages['lg_admin_save_changes']))?($pages['lg_admin_save_changes']) : 'Save';  ?></button>
