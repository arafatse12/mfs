		   			...
		   			$profile_img="assets/img/user.jpg";
						...
						<div class="mb-4">
							<div class="d-sm-flex flex-row flex-wrap text-center text-sm-left align-items-center">
								<img alt="profile image" src="<?php echo (settingValue('profile_img'))?base_url().settingValue('profile_img'):(base_url().$profile_img); ?>" class="avatar-lg rounded-circle">
								<div class="ml-sm-3 ml-md-0 ml-lg-3 mt-2 mt-sm-0 mt-md-2 mt-lg-0 info-blk-style">
									<?php 
                                        ...
                                        $this->db->where('name_id', $this->session->userdata('id'));
                                        $this->db->where('lang_type', $user_lang);
                                        $lang_pro = $this->db->get('users_lang')->row_array();
									?>
									<h6 class="mb-0"><?php echo (!empty($lang_pro['name'])) ? $lang_pro['name'] : $user->name; ?></h6>
						...
						<div class="widget settings-menu">
							<ul>
								...
								<li class="nav-item">
									<a href="<?php echo base_url()?>payout-settings" class="nav-link <?= ($this->uri->segment(1)=="payout-settings")?'active':'';?>" >
							...
							?>
								<li class="nav-item">
									<a href="<?php echo base_url()?>provider-change-password" class="nav-link <?= ($this->uri->segment(1)=="provider-change-password")?'active':'';?>">
										...
										<span><?php echo (!empty($user_language[$user_selected]['lg_change_password'])) ? $user_language[$user_selected]['lg_change_password'] : $default_language['en']['lg_change_password']; ?></span>
									</a>
								</li>
							<?php } ?>
							</ul>
