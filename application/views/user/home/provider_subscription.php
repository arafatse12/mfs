                    ...
                    if (empty($my_subscribe) || !empty($datas) && $datas['fee'] == 0 && date('Y-m-d', strtotime($my_subscribe['expiry_date_time']) >= date('Y-m-d'))) {
                        ...
                        <div class="col-xl-4 col-md-6 <?php echo $class; ?>">
                            <div class="card">
                                <div class="card-body">
                                    <div class="pricing-header">
                                        <?php 
                                            $sub_lang = ($this->session->userdata('user_select_language'))?$this->session->userdata('user_select_language'):'en';
                                            $this->db->where('sub_id', $list['id']);
                                        ...
                                        <p>
                                            <?php echo (!empty($user_language[$user_selected]['lg_Monthly_Price'])) ? $user_language[$user_selected]['lg_Monthly_Price'] : $default_language['en']['lg_Monthly_Price']; ?>
                                        </p>
                                        <?php } else { ?>
                                            <p>One Day - Free</p>
                                        <?php } ?>
                                    ...
                                    <div class="pricing-card-price">
                                       <?php //print_r($this->session->userdata('name'));exit ?>
                                        <h3 class="heading2 price"><?php echo currency_conversion($user_currency_code).$service_amount; ?></h3>
                                        <input type="hidden" name="currency_val" value="<?php echo$user_currency_code; ?>" id="currency_val">
                                        <input type="hidden" name="name" value="<?php echo $this->session->userdata('name'); ?>" id="name">
                                        <input type="hidden" name="email" value="<?php echo $this->session->userdata('email') ?>" id="email">
                                    ...
                                    <ul class="pricing-options">
                                        ...
                                        <li class="d-none"><i class="far fa-check-circle"></i> <?php echo $list['fee_description'];echo " "; echo (!empty($user_language[$user_selected]['lg_listing_submission'])) ? $user_language[$user_selected]['lg_listing_submission'] : $default_language['en']['lg_listing_submission']; ?></li>
									...
									<?php
                            			if ($list['id'] != $my_subscribe['subscription_id'] && ($list['fee'] != 0 && $list['fee'] != '')) {
                            		?>
									...
									<div class="row m-2">
                                        ...
                                        <?php } if(settingValue('paystack_show') == '1') { ?>
                                    ...
                                    <?php } if(!empty($midtrans_option_status)) { ?>
                                             
                                             <div class="col-6">
                                             <input class="form-check-input" type="radio" name="payment_type" id="midtrans"  value="midtrans">
                                                <img src="<?php echo base_url() . "assets/img/midtrans.png"; ?>">
                                             </div>
                                         <?php } if(settingValue('flutter_show') == '1') { ?>
                                             
                                             <div class="col-6">
                                             <input class="form-check-input" type="radio" name="payment_type" id="flutter"  value="flutter">
                                             <img src="<?php echo base_url() . "assets/img/flutterwave.png"; ?>">
                                             </div>
                                         <?php } if(settingValue('iyzico_show') == '1') { ?>
                                             
                                             <div class="col-6">
                                             <input class="form-check-input" type="radio" name="payment_type" id="iyzico"  value="iyzico">
                                             <img src="<?php echo base_url() . "assets/img/iyzico.png"; ?>">
                                             </div>
                                         <?php } 
                                    ...
                                    ?>
                                        <div class="col-6">
                                            ...
                                            <img src="<?php echo base_url() . "assets/img/bank-transfers.png"; ?>">
                                           <!--  BankTransfer -->
                                        </div>
                                    <?php } ?>
									</div>
							...
							<input type="hidden" name="order" value="<?php echo $list['subscription_name'] ?>">
									...
									<input type="hidden" id="midtrans_key" value="midtrans">
                                    
                                    <input type="hidden" id="flutter_apikey" value="<?php echo $flutter_apikey; ?>">

                                    <input type="hidden" id="iyzico_apikey" value="<?php echo $iyzico_apikey; ?>">
                                ...
                                <ul class="row">
                                    ...
                                    <li class="col-sm-4">
                                        ...
                                        <p><span class="amount"><?php
                                                ...
                                                ?> </span> 
                                                <?php if($my_subscribe['paid_status'] == 1) { ?>
                                                    <span class="badge bg-success-light"><?php echo (!empty($user_language[$user_selected]['lg_Paid'])) ? $user_language[$user_selected]['lg_Paid'] : $default_language['en']['lg_Paid']; ?></span>
                                                    <?php } else { ?>
                                                        <span class="badge bg-warning-light"><?php echo (!empty($user_language[$user_selected]['lg_Pending'])) ? $user_language[$user_selected]['lg_Pending'] : $default_language['en']['lg_Pending']; ?></span>
                                                    <?php } ?>
                                            </p>
                                ...
                                <table class="table table-center mb-0 no-footer">
                                    ...
                                    <tbody>
                                        ...
                                        <?php foreach ($my_subscribe_list as $row) { ?>
                                            ...
                                            <?php //echo '<pre>'; print_r($my_subscribe_list); exit;
                                            ...
                                            $sub_lang = ($this->session->userdata('user_select_language'))?$this->session->userdata('user_select_language'):'en';
                                            $this->db->where('sub_id', $row['id']);
                                            $this->db->where('lang_type', $sub_lang);
                                            $sub_name_table = $this->db->get('subscription_lang')->row_array();
                                            ...
                                            <tr role="row">
                                                ...
                                                <td><?php echo $sub_name_table['subscription_name']; ?></td>
...
<?php } ?>
            ...
            <!-- Paysolution Payment Form -->
                ...
                <form action="<?php echo base_url()?>vtweb/vtweb_checkout"  method="POST" id="midtrans_form">
                        
                        <input type="hidden" name="order_id" id="order_id" value="">
                        <input type="hidden" name="gross_amount" id="gross_amount" >
                        <!-- <input type="hidden" name="gross_amount" id="midtrans_amt" > -->
                        <input type="hidden" name="return_url" id="return_url" value="<?php echo site_url()?>user-wallet">
                        <input type="hidden" name="first_name" id="first_name" value="<?php echo $user_details['name']; ?>">
                        <input type="hidden" name="phone" id="phone" value="<?php echo $user_details['mobileno']; ?>">
                        <input type="hidden" name="email" id="email" value="<?php echo $user_details['email']; ?>">
                        <input type="hidden" id="address"  name="address" value="<?php echo (!empty($user_details['address'])) ? $user_details['address'] : "1234 Main St."; ?>">
                        <input type="hidden" id="city" value="<?php echo (!empty($city)) ? $city : "Chicago"; ?>">
                         <input type="hidden" name="productdetail" id="productdetail" value="subscription">
                         <input type="hidden" name="refno" id="refno" value="<?=uniqid()?>">
...
if ($flutter_option == 1) {
    $futter_key = settingValue('flutter_apikey');
} else {
    $flutter_key = settingValue('live_flutter_apikey');
}

if ($iyzico_option == 1) {
    $iyzico_key = settingValue('iyzico_apikey');
} else {
    $iyzico_key = settingValue('live_iyzico_apikey');
}
...
<input type="hidden" id="flutter_key" value="<?php echo $flutter_key; ?>">
<input type="hidden" id="iyzico_key" value="<?php echo $iyzico_key; ?>">
