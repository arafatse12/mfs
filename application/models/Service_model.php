...
class Service_model extends CI_Model
{
	...
	public function get_service()
	{
          ...
          $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,c.category_name,sc.subcategory_name,s.rating_count,s.service_location,s.url");
	...
	{
		$this->db->select("service_image,service_id,id");
    ...
    {      
     ...
     {
       $this->db->select("s.id,s.user_id,s.service_title,s.service_amount,s.mobile_image,c.category_name,c.category_image,r.rating,sc.subcategory_name,s.service_location,s.currency_code,s.url");
        $this->db->from('services s');
        $this->db->join('categories c', 'c.id = s.category', 'LEFT');
        $this->db->join('subcategories sc', 'sc.id = s.subcategory', 'LEFT');
     ...
     {
       $this->db->select("s.id,s.user_id,s.service_location,s.service_title,s.service_amount,s.mobile_image,s.currency,c.category_name,c.category_image,r.rating,sc.subcategory_name,s.url");
        $this->db->from('services s');
        $this->db->join('categories c', 'c.id = s.category', 'LEFT');
        $this->db->join('subcategories sc', 'sc.id = s.subcategory', 'LEFT');
        $this->db->join('rating_review r', 'r.service_id = s.id', 'LEFT');
    ...
    {
        return $this->db->where("status",1)->order_by("id", "DESC")->get('subscription_fee')->result_array(); //->where("status",1)
    }
    ...
    public function ratingstype_list()
    {
        ...
        public function get_user_listonly(){
            
          return $this->db->where('status',1)->where('id>=',1)->get('users')->result_array();
        ...
        public function get_service_list(){
            return $this->db->get('services')->result_array(); //where("status",1)->
    ...
    public function get_review()
    {
        $value=$this->db->select('tab_1.id,tab_1.rating,tab_1.review,tab_1.created')->
                        select('tab_2.name as user_name,tab_2.profile_img as user_image,tab_3.name as provider_name,tab_3.profile_img as provider_image,tab_4.service_title,tab_4.id as service_id,tab_5.name as type_name,tab_2.id as user_id,tab_3.id as provider_id')->
    ...
    public function get_review_filter($service,$provider,$user,$type,$from,$to)
    {
        ...
        $this->db->select('tab_2.name as user_name,tab_2.profile_img as user_image,tab_3.name as provider_name,tab_3.profile_img as provider_image,tab_4.service_title,tab_5.name as type_name, tab_4.id as service_id');
    ...
    //deleted service fileter
    public function delete_service_filter($service_title,$category,$subcategory,$from,$to)
    {
      if(!empty($from)) {
        $from_date=date("Y-m-d 00:00:00", strtotime($from));
      }else{
    ...
    //inactive service filter
    public function inactive_service_filter($service_title,$category,$subcategory,$from,$to)
    {
      if(!empty($from)) {
        $from_date=date("Y-m-d 00:00:00", strtotime($from));
      }else{
    ...
    public function check_provider_booking_list($id){
      $ret=$this->db->select('*')->
                      from('book_service')->
                      
                      where('provider_id',$id)->where_not_in('status',[5,7,6])->
                      get()->result();
    ...
    function edit_provider_data($id='') {
        if(empty($id)) {
            return false;
        }

        $result=$this->db->where('id',$id)->where('delete_status',0)->get('providers')->row_array();
        return $result;
    }

    //add service name
    public function add_service_name($service_id)
    {
        ...
        foreach ($languages as $language) {
            $data = array(
                ...
                'service_name' => $this->input->post('service_title_' . $language->id, true)
            );
            $this->db->insert('service_lang', $data);
        }
    }

    //update service name
    public function update_service_name($service_id)
    {
        $query = $this->db->query("select * from language WHERE status = '1'");
        $languages = $query->result();
        foreach ($languages as $language) {
            ...
            $row = $this->db->get('service_lang')->row();
            if (empty($row)) {
                $this->db->insert('service_lang', $data);
            } else {
                $this->db->where('service_id', $service_id);
                $this->db->where('lang_type', $language->language_value);
                $this->db->update('service_lang', $data);
            }
        }
    }
    ...
    {
      $result=$this->db->order_by('id','DESC')->get('refund_request')->result_array();
      return $result;   
    }

    public function pending_service_list($service_title='',$category='',$subcategory='',$from='',$to='') {
        if(!empty($service_title)) {
        ...
        $this->db->select('*');
        $this->db->from('services');
        $this->db->where('admin_verification!=',1);
        $this->db->order_by('id', 'DESC');
        return $this->db->get()->result_array();   
    }
    ...
    public function subscriptionlist_filter($user_name,$from,$to){
        if(!empty($from)) {
            $from_date=date("Y-m-d 00:00:00", strtotime($from));
        }else{
            $from_date='';
