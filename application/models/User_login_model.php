...
class User_login_model extends CI_Model
{
  ...
  public function signup($inputs)
  {
      ...
      //chat
      $chat_arr = ['sender_token'=>'0dreamsadmin', 'receiver_token'=>$unique_code, 'message'=>$chat_text, 'status'=>'1', 'read_status'=>'0', 'utc_date_time'=>date('Y-m-d H:i:s')];
  ...
  public function otp_check_mobile_no($inputs='')
   {
      ...
      $this->db->where(array('delete_status'=>0,'mobileno'=>$mobileno,'country_code'=>$inputs['countryCode']));
      return $this->db->count_all_results('providers');
   }
   ...
   {
      $this->db->where(array('country_code'=>$inputs['countryCode'],'status'=>1,'delete_status'=>0))->like('mobileno',$inputs['mobileno'],'match','after');
      return $this->db->count_all_results('providers');
   }
        ...
        if( $time_count ==1)
        {
         ...
         $this->db->update('mobile_otp', array('status'=>0));
          ...
          if($result)
          {
              ...
              $this->add_provider_name($user_id);
   ...
   //add user name
    public function add_user_name($user_id) {
        $query = $this->db->query("select * from language WHERE status = '1'");
        $languages = $query->result();

        $userName = $this->db->get_where('users', array('id'=>$user_id))->row()->name;
    ...
    //add provider name
    public function add_provider_name($provider_id) {
        $query = $this->db->query("select * from language WHERE status = '1'");
        $languages = $query->result();

        $providerName = $this->db->get_where('providers', array('id'=>$provider_id))->row()->name;
        ...
        foreach ($languages as $language) {
           ...
           //check provider name exists
            ...
            $this->db->where('name_id', $provider_id);
            $this->db->where('lang_type', $language->language_value);
            $row = $this->db->get('users_lang')->row();
            if (empty($row)) {
                $this->db->insert('users_lang', $data);
   ...
   public function get_provider_details($mobile_number)
    {
       ...
       $record = $this->db->where(array('mobileno'=>$mobile_number,'status'=>1))->get('providers')->row_array(); 
