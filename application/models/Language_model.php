...
class Language_model extends CI_Model {
    ...
    public function language_list($page_key,$lang_key){
    ...
    public function edit_appkeyword($id)
    {
        $query  = $this->db->query("SELECT * FROM `pages` WHERE `p_id` =  $id;");
        $result = $query->row();
        return $result;
	}
	...
	{
	  $this->db->select("lang_key, lang_value, language, type");
	  $this->db->where('language', 'en');
	  $this->db->from('language_management');
	  return $this->db->get();
	}

	public function fetch_app_data()
	{
	  $this->db->select("page_key, lang_key, lang_value, placeholder, validation1, validation2, validation3, language");
	  $this->db->where('language', 'en');
	  $this->db->from('app_language_management');
	  return $this->db->get();
	}

	public function check_language($lang_key,$language)
	{
	...
	public function check_app_language($page_key,$lang_key,$language)
	{
		$this->db->where('page_key', $page_key);
		$this->db->where('lang_key', $lang_key);
		$this->db->where('language', $language);
