        ...
        $(document).ready(function() {
            ...
            $(document).ready(function() {
                ...
                $('#contact_form1').bootstrapValidator({
                    ...
                    var phone = $('#phone').val();
                    var upload_cv = $('#upload_cv').val();
                    $.ajax({
                        type: 'POST',
                        url: base_url + 'user/job/insert_job',
                        data: { name: name, email: email, csrf_token_name: csrf_token, message: message, address: address, phone: phone, upload_cv: upload_cv },
                        success: function(response) {
                            if (response == 1) {
                                swal({
                                    title: "Message Send !",
